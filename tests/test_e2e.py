...
LLM_TEXT = ('{"narrative": "夜色下的训练场灯火通明，你深吸一口气，做出了决定。", '
...
async def fake_generate_stream(system, user, max_tokens=800):
    for i in range(0, len(LLM_TEXT), 25):
        yield LLM_TEXT[i:i + 25]


def _mock_narrator(monkeypatch):
...
def decision_done(text: str) -> dict:
    """解析决策 SSE 流，返回 (meta_payload, done_payload)。"""
    events = parse_sse(text)
    meta = next(p for ev, p in events if ev == "meta")
...
def test_full_career_16_to_40(client, monkeypatch):
    ...
    for step in range(600):  # 上限保护
        ...
        meta, done = decision_done(r.text)
        # 退役链路
        if meta.get("career_over"):
            assert meta["ended"] in ("退役", "转教练")
            assert max_age >= 40, "40 岁前不应退役"
            if meta["ended"] == "退役":
                assert isinstance(meta["honors"], list)
                assert isinstance(meta["milestones"], list)
                assert meta["career_stats"]
            else:
                assert meta["coach"]
            return meta
        p = meta["player"]
...
def test_all_positions_can_play(client, monkeypatch):
    ...
    for pos in POSITIONS:
        ...
        _, done = decision_done(r.text)
        assert done["player"]["age"] >= 17
...
def test_transfer_accept_updates_club(client, monkeypatch):
    ...
    p0 = r.json()["player"]
    offers = generate_offers(p0, world=build_world("data"), rng_seed=1)
    if not offers:  # 16 岁 ovr 低可能无报价——调引擎验证报价结构
        p0["ovr"] = 75
        offers = generate_offers(p0, world=build_world("data"), rng_seed=1)
